...
from Whatsapp_bot import get_bhashini_code, normalize_lang_code
...
class VoiceService:
    ...
    async def speech_to_text(
        ...
        Tries Bhashini first; on any failure falls back to Whisper.
        """
        if not audio_bytes:
            return ""
        ...
        if await self._use_bhashini():
            try:
                ...
                logger.warning("Bhashini STT failed (%s) – falling back to Whisper", exc)
    ...
    async def text_to_speech(
        ...
        Tries Bhashini first; on any failure falls back to gTTS.
        """
        text = (text or "").strip()
        if not text:
            return b""
        ...
        if not _GTTS_AVAILABLE:
            raise RuntimeError(
                "No TTS backend available (Bhashini failed and gTTS is not installed)"
            )
