...
def create_app(
    *,
    ...
    try:
        from backend.app.whatsapp_bot import router as whatsapp_router
        app.include_router(whatsapp_router)
        if hasattr(__import__("backend.app.whatsapp_bot"), "mount_media_route"):
    ...
    try:
        from backend.app.webhooks import router as webhooks_router
        app.include_router(webhooks_router)
    except ImportError as exc:
        logger.warning("Could not import webhooks: %s", exc)

    try:
        from backend.app.whatsapp_webhook import app as whatsapp_webhook_app
        app.mount("/whatsapp", whatsapp_webhook_app)
    except ImportError as exc:
        logger.warning("Could not import whatsapp_webhook: %s", exc)

    try:
        from backend.app.voice_webhook import app as voice_webhook_app
        app.mount("/voice", voice_webhook_app)
    except ImportError as exc:
        logger.warning("Could not import voice_webhook: %s", exc)

    try:
        from backend.app.sms_webhook import app as sms_webhook_app
    ...
    try:
        from backend.app.alert_dispatcher import get_alert_dispatcher
        get_alert_dispatcher()
        logger.info("Alert dispatcher initialised")
    except ImportError as exc:
        logger.warning("Could not import alert_dispatcher: %s", exc)

    # New modular routers under backend/app/routers/
    try:
        from backend.app.routers.data_sources import router as data_sources_router
        app.include_router(data_sources_router)
    except ImportError as exc:
        logger.warning("Could not import data_sources router: %s", exc)
    ...
    async def _startup() -> None:
        ...
        try:
            from backend.app.weather_service import get_weather_service
            get_weather_service()
        except Exception:
            pass
        try:
            ...
            pass
        try:
            from backend.app.webhooks import seed_demo_sessions
            seed_demo_sessions()
        except Exception:
            pass

        try:
            from backend.app.alert_generator import get_alert_generator
    ...
    async def _shutdown() -> None:
        ...
        try:
            from backend.app.alert_generator import close_alert_generator
